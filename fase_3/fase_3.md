...
- Um texto (ou marcador) para mostrar a nota sendo tocada no momento;
  - \color{purple}{Esta especificação foi removida, pois notamos que algumas notas dependendo da velocidade iria atualizar muito rapidamente perdendo sentido de tentar acompanhar todas as notas, dando uma sobrecarga visual.}
- Botão ou tecla de play;
...
Na Figura \ref{croqui} temos o croqui da interface do usuário. A idéia inicial é que os botões de play, pause e reiniciar, ficassem em baixo pela familiaridade de programas tradicionais de tocadores de música. A lista de comandos ficaria logo abaixo da entrada de texto para ficar proxima facilicando o acompanhamento durante a inserção do texto.

![Croqui da interface do usuário na Fase 1 \label{croqui}](../fase_1/croqui.png)

![Interface final do usuário com mensagem de erro \label{gui_error_box}](gui_final_error_box.png)

![Interface final do usuário sem mensagem de erro \label{gui}](gui_final.png)

Nas Figuras \ref{gui_error_box} e \ref{gui} temos a interface final do usuário (com e sem erro de input). O layout do programa se manteve muito proximo do planejado no croqui apenas adicionando os botões na nova requisição, feito a fusão do botão play e mouse.
Foi modificado tambem a localização dos componentes para uma melhorar a organização visual deixando seu visual mais equilibrado, e os elementos foram reposicionados deixando botoes de uso mais frequente proximos garantido a facilidade de uso do programa.
...
class UserInputWidget(QtWidgets.QWidget):
    ...
    def unblock(self) -> None:
        # Desbloqueia todas as formas de input do usuári
    ...
    def _on_ok_pressed(self) -> None:
        ...
        # Aqui é chamadaa função de validação.
...
class TxtLoaderWidget(QtWidgets.QWidget):
    file_loaded = pyqtSignal(str)  # Atributo público que sinaliza para um objeto externo
                                   # que o arquivo foi carregado com sucesso.
...
O painel de controle foi simplificado, de maneira que o processo de bloqueamento e desbloqueamento de botões foi unificado cada um em um método.

A classe `ControlBoard` também não desempenha nenhuma ação concreta em relação ààs ações de play, pause, restart e salvar, ela apenas _sinaliza_ que o usuário clicou nestes botões.
...
### Classes relacionadas ao Pocessamento para o arquivo MIDI

#### `MidiFileProcessor`

```python
class MidiFileProcessor:
    # Valores configurados como patdrão inicial
    _DEFAULT_VOLUME = ...
    _DEFAULT_OCTAVE = ...
    _MAX_VOLUME = ...
    _MAX_OCTAVE = ...
    ...
    def _process_note(self, char_to_midi, char):
      # Processa notas atravez de eventos midi de nota_on e nota_off
...
### Classes relacionadas à exibição do intrumento sendo tocado
...
class MidiInstrumentBox(QWidget):
    ...
    def set_instrument(self, instrument: int):
      # Atuaiza intrumento na GUI

    def _get_instrument_names(self):
      # Lista de intrumentos midi de 0 a 127
```
...
Para os testes manuais foram focados em pontos específicos da lista de comandos:

- Testes auditivos se as notas estão corretas;
- Se o incremento e decremento de oitavas condiz com sua sonolidade;
- O elemento de pausa está presente entre notas;
- Se os valores de aumento de velocidade, oitavas e volume não ultrapaça o limite max ou min (teste de fronteiras);
- Se os valores estão sendo resetados corretamente;
- Testes de casos especificos como repetição da nota ou o tocar do telefone para as letras i,u e o, procurando obter ambas variações.
